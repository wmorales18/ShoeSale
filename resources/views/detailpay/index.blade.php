	...
	<div class="container">
	  <div class="row">
	    <div class="col s10 m6 l6">
	      <h5 class="breadcrumbs-title">Detalle Pago</h5>
	...
	<div class="row">
	  <div class="col s12">
	    <table id="data-table-simple" class="responsive-table display" cellspacing="0">
	      ...
	      <tbody>
	      	@foreach($detailpays as $detailpay)
	        <tr>
	          <td>{{$detailpay->id}}</td>
	          <td>{{$detailpay->total}}</td>
	          <td>{{$detailpay->typepays->name}}</td>
	          <td>{{$detailpay->bills->id}}</td>
	          <td>
	          	<div class="form-group col s12">

                    <form action="{{route('detailpay.destroy',$detailpay->id)}}" method="POST" style="display: inline">
                        {{method_field('DELETE')}}
                        @csrf
                        <a class="btn-flat" href="{{ route('detailpay.show',$detailpay->id) }}">
                            <i class="material-icons indigo-text">visibility</i>
                        </a>
                        <a class="btn-flat" href="{{ route('detailpay.edit',$detailpay->id) }}">
...
<div class="fixed-action-btn " style="bottom: 50px; right: 19px;">
  ...
  <ul>
    <li>
      <a href="{{route('detailpay.create')}}" class="btn-floating blue">
