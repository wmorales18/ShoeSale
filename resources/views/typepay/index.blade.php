	...
	<div class="container">
	  <div class="row">
	    <div class="col s10 m6 l6">
	      <h5 class="breadcrumbs-title">Tipo Pago</h5>
	    ...
	    <table id="data-table-simple" class="responsive-table display" cellspacing="0">
	      ...
	      <tbody>
	        ...
	        <tr>
	          <td>{{$typepay->id}}</td>
	          <td>{{$typepay->name}}</td>
	          <td>{{$typepay->description}}</td>
	           <td>
	          	<div class="form-group col s12">

                    <form action="{{route('typepay.destroy',$typepay->id)}}" method="POST" style="display: inline">
                        {{method_field('DELETE')}}
                        @csrf
                        <a class="btn-flat" href="{{ route('typepay.show',$typepay->id) }}">
                            <i class="material-icons indigo-text">visibility</i>
                        </a>
                        <a class="btn-flat" href="{{ route('typepay.edit',$typepay->id) }}">
...
<div class="fixed-action-btn " style="bottom: 50px; right: 19px;">
  ...
  <ul>
    <li>
      <a href="{{route('typepay.create')}}" class="btn-floating blue">
