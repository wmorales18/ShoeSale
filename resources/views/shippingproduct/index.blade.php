	...
	<div class="container">
	  <div class="row">
	    <div class="col s10 m6 l6">
	      <h5 class="breadcrumbs-title">Producto</h5>
	      <ol class="breadcrumbs">
	        <li><a href="/">Home</a></li>	        
	        <li class="active">Envio Producto</li>
	      </ol>
	    </div>
	  </div>
	</div>
	  ...
	  <div class="col s12">
	    <table id="data-table-simple" class="responsive-table display" cellspacing="0">
	      ...
	      <tbody>
	        ...
	        <tr>
	          <td>{{$shippingproduct->id}}</td>
	          <td>{{$shippingproduct->quantity}}</td>
	          <td>{{$shippingproduct->typeshippings->name}}</td>
	          <td>{{$shippingproduct->shippings->id}}</td>
	          <td>{{$shippingproduct->productinventories->products->name}}</td>
	          <td>
	          	<div class="form-group col s12">

                    <form action="{{route('shippingproduct.destroy',$shippingproduct->id)}}" method="POST" style="display: inline">
                        {{method_field('DELETE')}}
                        @csrf
                        <a class="btn-flat" href="{{ route('shippingproduct.show',$shippingproduct->id) }}">
                            <i class="material-icons indigo-text">visibility</i>
                        </a>
                        <a class="btn-flat" href="{{ route('shippingproduct.edit',$shippingproduct->id) }}">
...
<div class="fixed-action-btn " style="bottom: 50px; right: 19px;">
  ...
  <ul>
    <li>
      <a href="{{route('shippingproduct.create')}}" class="btn-floating blue">
