	...
	<div class="container">
	  <div class="row">
	    <div class="col s10 m6 l6">
	      <h5 class="breadcrumbs-title">Tipo Activo</h5>
	  ...
	  <div class="col s12">
	    <table id="data-table-simple" class="responsive-table display" cellspacing="0">
	      ...
	      <tbody>
	        ...
	        <tr>
	          <td>{{$typeactive->id}}</td>
	          <td>{{$typeactive->name}}</td>
	          <td>{{$typeactive->description}}</td>
	           <td>
	          	<div class="form-group col s12">

                    <form action="{{route('typeactive.destroy',$typeactive->id)}}" method="POST" style="display: inline">
                        {{method_field('DELETE')}}
                        @csrf
                        <a class="btn-flat" href="{{ route('typeactive.show',$typeactive->id) }}">
                            <i class="material-icons indigo-text">visibility</i>
                        </a>
                        <a class="btn-flat" href="{{ route('typeactive.edit',$typeactive->id) }}">
...
<div class="fixed-action-btn " style="bottom: 50px; right: 19px;">
  ...
  <ul>
    <li>
      <a href="{{route('typeactive.create')}}" class="btn-floating blue">
